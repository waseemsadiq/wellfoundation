<div class="flex flex-col w-full p-0 font-medium text-left text-gray-700">
    ...
    <p class="flex flex-col ml-3">
        ...
        <a <? if($item->route) : ?>href="<?= route($item->route); ?>"<? else: ?>rel="nofollow" href="<?= $item->url ?>"<? endif ?> class="block text-sm py-2 leading-none rounded border-white hover:border-transparent text-gray-500 hover:text-indigo-700 hover:bg-white mt-4 md:mt-0">
            <?= $item->name; ?>
        </a>
        <? } ?>
    </p>
